...
class DogFilter(BaseFilter):
    ...
    def apply_filter(self, img, face, keypoint_pred, face_x, face_y, face_width, face_height):
        nose_x, nose_y = self.get_nose_row_col(keypoint_pred)
        left_eye_center_x, left_eye_center_y, right_eye_center_x, right_eye_center_y = self.get_eye_cood_for_dog(
            keypoint_pred, face)
        resized_dog_filter = self.get_resized_dog_filter(face, left_eye_center_x, right_eye_center_x)
        return self.overlay_dog_filter(resized_dog_filter, img, nose_x, nose_y, face_x, face_y, face_width, face_height)

    def get_nose_row_col(self, keypoint_pred):
        normalized_nose_row = keypoint_pred[21]
        normalized_nose_col = keypoint_pred[22]
        return PixelPoint.denormalize_point(normalized_nose_row), PixelPoint.denormalize_point(normalized_nose_col)
    ...
    def get_resized_dog_filter(self, face, left_eye, right_eye):
        face_height = face.shape[0]
        filter_width = 2 * abs(right_eye - left_eye)
        return cv2.resize(self.dog_filter, (int(filter_width), int(2 * face_height)))
            ...
            for col in range(0, dog_filter.shape[1]):
                for channel in range(0, dog_filter.shape[2]):
                    if dog_filter[row][col][channel] > 0:
                        img_col = int(col + nose_y + y - face_width / 2 + dog_filter.shape[0] * DOG_FILTER_X_OFFSET)
                        img_row = int(row + nose_x + x - face_height / 2 + dog_filter.shape[1] * DOG_FILTER_Y_OFFSET)
